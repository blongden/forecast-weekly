...
def fetch_midprice(date_from: date, date_to: date) -> int:
    ...
    while chunk_start <= date_to:
        ...
        # The API treats `to` as exclusive (returns only the midnight slot for
        # the `to` date).  Request one extra day so chunk_end gets all 48 slots.
        api_to = chunk_end + timedelta(days=1)
        resp = requests.get(
            BASE_URL,
            params={
